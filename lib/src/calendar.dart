...
/// Callback exposing currently selected week
typedef void OnWeekSelected(DateTime day, List events, List holidays);
...
/// Selection type Select day only or week
enum SelectionType { day, week }
...
class TableCalendar extends StatefulWidget {
  ...
  /// Called whenever any day gets tapped.
  final OnWeekSelected onWeekSelected;
  ...
  /// If nothing is provided, a default `day` will be used.
...
class _TableCalendarState extends State<TableCalendar>
  ...
  void initState() {
    ...
    widget.calendarController._init(
      ...
      selectedWeekCallback: _selectedWeekCallback,
  ...
  void _selectedDayCallback(DateTime day) {
    if (widget.onDaySelected != null &&
        widget.selectionType == SelectionType.day) {
  ...
  void _selectedWeekCallback(DateTime day) {
    if (widget.onWeekSelected != null &&
        widget.selectionType == SelectionType.week) {
      List<dynamic> eventsInWeek = [];
      List<dynamic> holidaysInWeek = [];
      for (DateTime weekDay in widget.calendarController.selectedWeek) {
        eventsInWeek +=
            widget.calendarController.visibleEvents[_getEventKey(weekDay)] ??
                [];
        holidaysInWeek += widget
                .calendarController.visibleHolidays[_getHolidayKey(weekDay)] ??
            [];
      }
  ...
  void _selectWeek(DateTime day) {
    if (widget.selectionType == SelectionType.week) {
      setState(() {
        widget.calendarController.setSelectedWeek(day, isProgrammatic: false);
        _selectedWeekCallback(day);
      });
    }
  }
  ...
  Widget _buildCell(DateTime date) {
    ...
    return GestureDetector(
      ...
      onTap: () => _isDayUnavailable(date)
          ...
          : {_selectDay(date), _selectWeek(date)},
  ...
  Widget _buildCellContent(DateTime date) {
    ...
    final tIsWeekSelected = widget.calendarController.isWeekSelected(date);
    ...
    final isWeekSelected =
        widget.builders.selectedWeekBuilder != null && tIsWeekSelected;
    ...
    if (isUnavailable) {
      ...
      return widget.builders.selectedWeekBuilder(
          context, date, widget.calendarController.visibleEvents[eventKey]);
